...
try:
    ...
    if latest_trade and SYMBOL in latest_trade:
        ...
        spy_price = safe_float(
            latest_trade[SYMBOL].price
        )

except Exception as e:

    log_activity(
        f"SPY latest trade unavailable • {e}",
        "warn",
    )
...
def normalize_stock_bars(raw_bars):
    ...
    # --------------------------------------------------------
    # Handle MultiIndex returned by Alpaca.
    # --------------------------------------------------------
        ...
        if symbol_level is not None:
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        "Unable to isolate SPY from Alpaca "
                        f"bar response: {e}"
                    )

        else:

            # Fallback for unnamed MultiIndex.
            found = False
...
def analyze_market():
    ...
    # --------------------------------------------------------
    # Explicit UTC historical window.
    # --------------------------------------------------------
    ...
    try:
        ...
        raise ValueError(
            "Alpaca SPY historical-data request failed: "
            f"{e}"
        )
    ...
    if response is None:
        ...
        raise ValueError(
            "Alpaca returned no SPY market-data response."
        )
    ...
    try:
        ...
        raise ValueError(
            f"Unable to read Alpaca SPY bar dataframe: {e}"
        )

    if raw_df is None:

        raise ValueError(
            "Alpaca returned no SPY dataframe."
        )

    if raw_df.empty:

        raise ValueError(
            "No SPY market data returned by Alpaca "
    ...
    if len(bars) < 60:
        ...
        raise ValueError(
            ...
            "were returned. At least 60 bars are required "
            "for the configured technical analysis."
        )
    ...
    #
    # IMPORTANT:
    # These values all come from the SAME historical dataset.
    # --------------------------------------------------------
...
def scan_options():
    ...
    # Use the technical scan's latest historical price when
    # available. Otherwise use the latest IEX trade.
