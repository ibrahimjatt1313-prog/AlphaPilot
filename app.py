...
def calculate_ai_decision(
    ...
    # ========================================================
    # SMART WEIGHTED TECHNICAL DECISION ENGINE
    # ========================================================

    price = float(market.get("price", 0) or 0)
    sma20 = float(market.get("sma20", 0) or 0)
    sma50 = float(market.get("sma50", 0) or 0)
    rsi = float(market.get("rsi", 50) or 50)
    macd = float(market.get("macd", 0) or 0)
    macd_signal = float(market.get("macd_signal", 0) or 0)
    volume = float(market.get("volume", 0) or 0)
    volume_average = float(market.get("volume_average", 0) or 0)
    ...
    bullish_confidence = sum(
        bullish_weights[name]
        for name, passed_check in bullish_checks.items()
        if passed_check
    )
    ...
    bearish_confidence = sum(
        bearish_weights[name]
        for name, passed_check in bearish_checks.items()
        if passed_check
    )

    bullish_passed = sum(
        bool(value)
        for value in bullish_checks.values()
    )

    bearish_passed = sum(
        bool(value)
        for value in bearish_checks.values()
    )
    ...
    checks = bullish_checks
    passed = bullish_passed

    # BUY requires:
    # - minimum configured confidence
    # - bullish trend structure
    # - at least 3 bullish conditions
    #
    # SELL is informational for now because the current
    # execution engine is Long Call / BUY only.

    if (
        bullish_confidence >= MIN_CONFIDENCE
        and bullish_passed >= 3
        and price > sma20
        and sma20 > sma50
    ):
        signal = "BUY"
        ...
        bearish_confidence >= MIN_CONFIDENCE
        and bearish_passed >= 3
        and price < sma20
        and sma20 < sma50
    ):
        signal = "SELL"
        confidence = bearish_confidence
        checks = bearish_checks
...
def run_risk_check():
    ...
    # Risk-sized quantity: never exceed either account-risk or exposure limit.
    # Also respect the configured default contract quantity as a ceiling.
    quantity = min(
...
if run_risk:
    try:
        ...
        # Step 3: Option selection is allowed only after a BUY signal.
        if st.session_state.ai_decision.get("signal") != "BUY":
            st.session_state.selected_option = None
            st.session_state.option_candidates = []
            st.session_state.risk_result = None
            st.error("Risk Check blocked: AI signal is NO TRADE.")
        elif st.session_state.get("selected_option") is None:
